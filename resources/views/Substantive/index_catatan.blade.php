				...
				<div class="panel-body">
					...
					<form id="data-all" enctype="multipart/form-data">
						...
						<table id="myTable" class="table table-striped table-bordered table-td-valign-middle">
							...
							<tbody>
								@foreach(substantive_catatan_get() as $no=>$data)
									<tr class="odd gradeX">
										...
										<td>
											...
											@else
												<span onclick="create(`{{coder($data->id)}}`)" class="btn btn-blue active btn-xs">Catatan</span> 
											@endif
...
@push('ajax')
	...
	<script>
		...
		function cetak(id){
			window.open("{{url('Substantive/Cetakcatatan')}}?id="+id, '_blank');
		}
		function send_to(a){
			
			$.ajax({
				type: 'GET',
				url: "{{url('Substantive/send_catatan_to_pengawas')}}",
				data: "id="+a,
				beforeSend: function() {
		...
		function create(a){
			location.assign("{{url('/Substantive/Catatan')}}?id="+a);
		}
