					...
					<ul class="nav nav-tabs">
						<li class="nav-item">
							<a href="javascript:;"  class="nav-link active">
								<span class="d-sm-none">SUBSTANTIVE PROGRAM</span>
								<span class="d-sm-block d-none">SUBSTANTIVE PROGRAM</span>
							</a>
						</li>
					</ul>
												...
												<tr>
													...
													<td>
														<table class="table table-bordered">
															...
															<tbody>
																@foreach(langkah_substantive($prog->id) as $la=>$langkah)
																	<tr>
																		<td>{{$la+1}}</td>
																		<td>{{$langkah->name}}</td>
			...
			<div class="modal-dialog" id="modal-sedeng">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<form id="tambah-data" action="{{url('/substantive')}}" method="post" enctype="multipart/form-data">
							@csrf
			...
			<div class="modal-dialog" id="modal-sedeng">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<form id="tambah-langkah" action="{{url('/substantive/langkah')}}" method="post" enctype="multipart/form-data">
							@csrf
							<input type="hidden" name="substantive_id" id="substantive_id" >
							<div class="col-xl-10 offset-xl-1">
								<div class="form-group row m-b-10" >
...
@push('ajax')
	...
	<script>
		$(document).ready(function() {
            ...
            $.ajax({
				type: 'GET',
				url: "{{url('Substantive/tampil_langkah_kerja')}}",
				data: "id={{$data->id}}",
		...
		function hapus(a){
			$.ajax({
				type: 'GET',
				url: "{{url('Substantive/hapus_pokok')}}",
				data: "id="+a,
				success: function(msg){
					location.reload();
				}
		...
		function hapus_langkah(a){
			$.ajax({
				type: 'GET',
				url: "{{url('Substantive/hapus_langkah')}}",
				data: "id="+a,
				success: function(msg){
					location.reload();
				}
