				...
				<div class="panel-body">
					...
					<form id="data-all" enctype="multipart/form-data">
						...
						<table id="myTable" class="table table-striped table-bordered table-td-valign-middle">
							...
							<tbody>
								@foreach(compliance_catatan_get() as $no=>$data)
									<tr class="odd gradeX">
...
@push('ajax')
	...
	<script>
		...
		$('#alasan').hide();
		function cetak(id){
			window.open("{{url('Compliance/Cetakcatatan')}}?id="+id, '_blank');
		}
		...
		function send_to(a){
			...
			$.ajax({
				type: 'GET',
				url: "{{url('Compliance/send_catatan_to_pengawas')}}",
				data: "id="+a,
				beforeSend: function() {
		...
		function create(a){
			location.assign("{{url('/Compliance/Catatan')}}?id="+a);
		}
