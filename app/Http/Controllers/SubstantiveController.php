...
class SubstantiveController extends Controller
{
    ...
    public function proses_catatan(request $request){
        ...
        if (isset($error)) {echo '<p style="padding:5px;color:#000;font-size:13px"><b>Error</b>: <br />'.implode('<br />', $error).'</p>';} 
            ...
            $size = $image->getSize();
            $imageFileName ='catatanlangkahsubs'.$request->id.'.'. $image->getClientOriginalExtension();
    ...
    public function cetak(request $request){
        ...
        $pdf = PDF::loadView('Substantive.cetak', compact('data'));
        $pdf->setPaper('A4', 'Potrait');
        return $pdf->stream();
    }
    public function cetakcatatan(request $request){
        $data=Audit::where('id',$request->id)->first();
        $pdf = PDF::loadView('Substantive.cetak_catatan', compact('data'));
        $pdf->setPaper('A4', 'Potrait');
        return $pdf->stream();
    }
